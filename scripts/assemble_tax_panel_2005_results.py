"""Append the certified tax_panel_2005 (2005 Tax Panel simple deduction) rows to the results panel.

Consumes the aggregated anchor cells produced by the certified scoring run
(``tmp/tax_panel_2005_rows_dollars.json`` — dollar aggregates computed against
the same rebuilt datasets' baselines by the local full-H5 aggregation), scales to
billions, interpolates non-anchor years linearly between anchors following the
panel convention, and writes the tax_panel_2005 rows into both tracked copies of
``results.csv`` (repo root and ``dashboard/public/data``). Idempotent: existing
tax_panel_2005 rows are replaced.
...
CELLS = REPO / "tmp" / "tax_panel_2005_rows_dollars.json"
...
def load_anchor_rows() -> dict[int, dict[str, object]]:
    ...
    for cell in cells:
        ...
        lineage = json.loads((REPO / "tmp" / "tax_panel_2005_lineage.json").read_text())
        cell_lineage = lineage[str(year)]
        row.update(
            {
                "reform_name": "tax_panel_2005",
                "scoring_type": "static",
    ...
    if missing:
        raise RuntimeError(f"tax_panel_2005 anchors missing: {missing}; refusing assembly.")
    return rows
...
def main() -> int:
    ...
    for target in TARGETS:
        with target.open() as file:
            ...
            kept = [row for row in reader if row["reform_name"] != "tax_panel_2005"]
        formatted = []
        ...
        print(f"wrote {len(formatted)} tax_panel_2005 rows into {target}")
    return 0
