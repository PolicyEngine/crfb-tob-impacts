...
def main():
    ...
    try:
        ...
        print(
            f"      ✓ Baseline calculated: ${baseline_revenue / 1e9:.2f}B (total: {baseline_time:.1f}s)"
        )
    ...
    for i, reform_id in enumerate(reform_ids, start=1):
        ...
        print(
            f"      DIAGNOSTIC: Starting reform {reform_id} at {time.strftime('%H:%M:%S')}"
        )
        ...
        try:
            ...
            if scoring_type == "static":
                ...
                print(
                    f"      DIAGNOSTIC: Looking up dynamic dict function for '{reform_id}'..."
                )
                ...
                print(
                    f"      DIAGNOSTIC: Found dynamic dict function: {dynamic_dict_func.__name__}"
                )
                ...
                print(
                    f"      DIAGNOSTIC: Got reform parameters dictionary with {len(reform_params)} keys"
                )

                # Create single reform from complete parameters
                print(
                    f"      DIAGNOSTIC: Creating Reform.from_dict() with {len(reform_params)} parameters..."
                )
            ...
            create_start = time.time()
            print(
                f"      DIAGNOSTIC: Calling Microsimulation(reform=<reform>, dataset='{dataset_name}')..."
            )
            ...
            print(
                f"      DIAGNOSTIC: About to calculate reform TOB revenue variables..."
            )
            tob_start = time.time()
            ...
            if reform_id in ["option5", "option6"]:
                print(
                    f"      DIAGNOSTIC: Calculating employer payroll revenue variables..."
                )
                emp_start = time.time()
                try:
                    ...
                    print(
                        f"        - Employer payroll revenue variables calculated: {emp_time:.1f}s"
                    )
                    print(
                        f"        - Employer SS tax revenue: ${employer_ss_revenue / 1e9:.2f}B"
                    )
                    print(
                        f"        - Employer Medicare tax revenue: ${employer_medicare_revenue / 1e9:.2f}B"
                    )
                except Exception as emp_error:
                    print(
                        f"        - Warning: Could not calculate employer payroll variables: {emp_error}"
                    )
            ...
            tob_medicare_impact = (
                reform_tob_medicare_revenue - baseline_tob_medicare_revenue
            )
            ...
            if reform_id in ["option5", "option6"]:
                ...
                if reform_id == "option5":
                    ...
                    if year >= 2033:
                        ...
                        print(
                            f"        - Option 6 phase-in: {rate * 100:.1f}% = {total_pp:.2f}pp, OASDI share: {(6.2 / total_pp if total_pp > 6.2 else 1.0) * 100:.1f}%"
                        )
                ...
                print(
                    f"        - OASDI gain: ${oasdi_gain / 1e9:+.2f}B, loss: ${oasdi_loss / 1e9:.2f}B, net: ${oasdi_net / 1e9:+.2f}B"
                )
                print(
                    f"        - HI gain: ${hi_gain / 1e9:+.2f}B, loss: ${hi_loss / 1e9:.2f}B, net: ${hi_net / 1e9:+.2f}B"
                )

            reform_time = time.time() - reform_start
            print(f"      ✓ Reform revenue: ${reform_revenue / 1e9:.2f}B")
            print(
                f"      ✓ Impact: ${impact / 1e9:+.2f}B ({reform_time:.1f}s total, {sim_time:.1f}s simulation)"
            )
            print(
                f"        - TOB Medicare HI impact: ${tob_medicare_impact / 1e9:+.2f}B"
            )
            ...
            if len(reform_ids) > 1:
                try:
                    ...
                    print(
                        f"      ✓ Incremental save to gs://{bucket_name}/{csv_path} ({len(results)}/{len(reform_ids)} reforms)"
                    )
                except Exception as save_error:
                    print(
                        f"      ⚠ Warning: Failed to save intermediate results: {save_error}"
                    )
