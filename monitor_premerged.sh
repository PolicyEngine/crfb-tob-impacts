#!/bin/bash
JOB_ID="years-20251031-140010-436a4f"
RESULTS_DIR="results/3years_premerged"

mkdir -p "$RESULTS_DIR"

echo "================================================================================"
echo "MONITORING 3-YEAR PRE-MERGED DYNAMIC DICT TEST"
echo "================================================================================"
echo "Job ID: $JOB_ID"
echo "Approach: Pre-merged reform dictionaries with CBO elasticities"
...
for i in {1..30}; do
    ...
    if [ -n "$DURATION" ]; then
        ...
        # Highlight when we pass critical thresholds
        if [ "$MINUTES" -eq 3 ] && [ "$SECS" -ge 0 ]; then
            echo "⚠️  CRITICAL: Passing 3-minute mark (previous failure point)..."
        fi
        if [ "$MINUTES" -eq 4 ] && [ "$SECS" -ge 0 ]; then
            echo "⚠️  CRITICAL: Passing 4-minute mark (reform chaining failure point)..."
        fi
    else
