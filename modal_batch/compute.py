...
def compute_year(
    ...
    print(
        f"    Baseline: ${baseline_revenue / 1e9:.2f}B ({time.time() - baseline_start:.1f}s)"
    )
    print(
        f"    TOB - OASDI: ${baseline_tob_oasdi_revenue / 1e9:.2f}B, Medicare HI: ${baseline_tob_medicare_revenue / 1e9:.2f}B"
    )
    ...
    for i, reform_id in enumerate(reform_ids, start=1):
        ...
        try:
            ...
            print(
                f"    OASDI: ${tob_oasdi_impact / 1e9:+.2f}B, Medicare HI: ${tob_medicare_impact / 1e9:+.2f}B"
            )
        ...
        for year in test_years:
            result = next(
                (
                    ...
                    if r["reform_name"] == reform_id and r["year"] == year
                ),
                None,
            )
            if result:
                print(
                    f"  {year}: ${result['revenue_impact'] / 1e9:+.2f}B (OASDI: ${result['tob_oasdi_impact'] / 1e9:+.2f}B)"
                )
    ...
    try:
        for result_batch in compute_year.starmap(args):
            ...
            if result_batch:
                ...
                print(
                    f"  [{completed}/{len(year_list)}] Year {year} saved to {year_file}"
                )
    except Exception as e:
        print(f"\n{'=' * 60}")
...
def _combine_results(output_dir, output_path, reform_list):
    ...
    for reform_id in reform_list:
        ...
        if len(reform_df) > 0:
            ...
            print(
                f"{reform_id}: ${total_impact:+,.1f}B total (OASDI: ${oasdi_impact:+,.1f}B, HI: ${hi_impact:+,.1f}B)"
            )
...
def list_volume_results():
    ...
    result = subprocess.run(
        ["modal", "volume", "ls", "crfb-results"], capture_output=True, text=True
    )
    print(result.stdout)
    if result.stderr:
