...
@pytest.mark.skipif(
    not os.path.exists("jupyterbook/revenue-impacts.ipynb"), reason="Notebook not found"
)
def test_revenue_impacts_notebook_has_outputs():
    """Test that revenue-impacts notebook has chart outputs."""
    cells_with_outputs, plotly_outputs = check_notebook_outputs(
        "jupyterbook/revenue-impacts.ipynb"
    )
    assert plotly_outputs > 0, (
        "Revenue impacts notebook needs to be executed with charts"
    )


@pytest.mark.skipif(
    not os.path.exists("jupyterbook/household-impacts.ipynb"),
...
def test_household_impacts_notebook_has_outputs():
    ...
    assert plotly_outputs > 0, (
        "Household impacts notebook needs to be executed with charts"
    )
