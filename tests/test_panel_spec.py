...
def test_static_plus_behavioral_cell_count_is_not_per_year_behavioral():
    years = list(default_selected_years())
    cells = wanted_cell_keys(PANEL_REFORMS, years, ("static", "behavioral"))
...
def test_behavioral_only_needs_only_endpoint_baselines():
    years = list(default_selected_years())

    assert needed_baseline_years(years, ("behavioral",)) == [2026, 2100]
    assert needed_baseline_years(years, ("static", "behavioral")) == years


def test_non_endpoint_behavioral_cell_is_refused():
    with pytest.raises(ValueError, match="endpoint-only"):
        validate_scoring_year("behavioral", 2030)

    validate_scoring_year("behavioral", 2026)
    validate_scoring_year("behavioral", 2100)
...
def test_cell_keys_keep_static_and_behavioral_outputs_separate():
    ...
    assert cell_key("option1", 2026, "behavioral") == "option1_2026_behavioral"


def test_conventional_label_is_rejected_before_scheduling():
    with pytest.raises(ValueError, match="Unknown scoring_type"):
        years_for_scoring("conventional", [2026, 2100])
