...
def test_cbo_elasticities_use_current_policyengine_us_contract():
    assert "gov.simulation.labor_supply_responses.elasticities.income" in CBO_ELASTICITIES
    assert (
        "gov.simulation.labor_supply_responses.elasticities.substitution.by_position_and_decile.secondary"
        in CBO_ELASTICITIES
    )
    assert "gov.simulation.labor_supply_responses.elasticities.income.base" not in CBO_ELASTICITIES
...
def test_conventional_option_dicts_embed_current_policyengine_us_elasticities():
    for conventional_dict in (
        get_option1_conventional_dict(),
        get_option12_conventional_dict(),
    ):
        assert (
