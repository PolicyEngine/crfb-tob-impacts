...
class TestTrustFundRevenue:
    ...
    def test_option2_vs_baseline_differ(self):
        ...
        income_tax_option2 = option2_sim.calculate(
            "income_tax", map_to="household", period=year
        )

        # Baseline simulation
        baseline_sim = Microsimulation()
        income_tax_baseline = baseline_sim.calculate(
            "income_tax", map_to="household", period=year
        )

        # Verify Option 2 has higher taxable SS than baseline
        taxable_ss_option2 = option2_sim.calculate(
            "taxable_social_security", period=year
        )
        taxable_ss_baseline = baseline_sim.calculate(
            "taxable_social_security", period=year
        )
