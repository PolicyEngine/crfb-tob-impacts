...
from src.runtime_config import dataset_path, validate_dataset_contract
...
def _write_dataset(
    ...
    *,
    ...
    tax_assumption_name: str = "trustees-core-thresholds-v1",
...
def test_dataset_path_rejects_wrong_tax_assumption(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CRFB_PROJECTED_DATASETS_SNAPSHOT_PATH", str(snapshot_dir))
    monkeypatch.setenv(
        "CRFB_REQUIRED_TAX_ASSUMPTION", "trustees-core-thresholds-v2"
    )
    monkeypatch.delenv("CRFB_DATASET_TEMPLATE", raising=False)

    with pytest.raises(ValueError, match="expected 'trustees-core-thresholds-v2'"):
        dataset_path(2026)
...
def test_validate_dataset_contract_rejects_wrong_tax_assumption(tmp_path):
    ...
    with pytest.raises(ValueError, match="expected 'trustees-core-thresholds-v2'"):
        validate_dataset_contract(
            ...
            required_tax_assumption="trustees-core-thresholds-v2",
