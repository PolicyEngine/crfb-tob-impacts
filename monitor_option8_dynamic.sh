#!/bin/bash
# Monitor option8 dynamic 75-year run
JOB_ID="$1"
REGION="${2:-us-central1}"
RESULTS_DIR="results/option8_75years_dynamic"
...
echo "================================================================================"
echo "MONITORING OPTION8 DYNAMIC - 75 YEARS (2026-2100)"
echo "================================================================================"
echo "Job ID: $JOB_ID"
echo "Region: $REGION"
echo "Reform: option8"
...
for i in {1..120}; do
    ...
    RESULT_FILES=$(ls "$TEMP_DIR"/*_option8_dynamic_results.csv 2>/dev/null | wc -l | tr -d ' ')

    if [ "$RESULT_FILES" -gt 0 ]; then
        echo "reform_name,year,baseline_revenue,reform_revenue,revenue_impact,scoring_type" > "$TEMP_DIR/merged.csv"
        tail -n +2 -q "$TEMP_DIR"/*_option8_dynamic_results.csv 2>/dev/null | sort -t',' -k2 -n >> "$TEMP_DIR/merged.csv"
...
PYEOF
    ...
    if [ "$STATE" = "SUCCEEDED" ]; then
        echo "✅ OPTION8 DYNAMIC COMPLETED! ($SUCCEEDED/75 succeeded, $FAILED failed)"
        break
    fi
